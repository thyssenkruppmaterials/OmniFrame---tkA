-- =========================================================================
-- Team Performance Optimization Migration
-- Created: January 3, 2026
-- Purpose: Eliminate N+1 queries by creating aggregated RPC functions
-- Impact: Reduces ~850 queries per day view to 3-5 queries
-- =========================================================================

-- =========================================================================
-- PART 1: COMPOSITE INDEXES FOR PERFORMANCE
-- These indexes optimize the date range queries used by the RPC functions
-- =========================================================================

-- Inbound scans composite index
CREATE INDEX IF NOT EXISTS idx_rr_inbound_scans_org_user_date 
ON rr_inbound_scans(organization_id, scanned_by, scanned_at);

-- Putaway operations composite index
CREATE INDEX IF NOT EXISTS idx_rf_putaway_org_created_date 
ON rf_putaway_operations(organization_id, created_by, created_at);

CREATE INDEX IF NOT EXISTS idx_rf_putaway_org_confirmed_date 
ON rf_putaway_operations(organization_id, confirmed_by, confirmed_at);

-- Outbound data composite indexes (multiple columns used for different actions)
CREATE INDEX IF NOT EXISTS idx_outbound_org_picked_date 
ON outbound_to_data(organization_id, picked_by, picked_at);

CREATE INDEX IF NOT EXISTS idx_outbound_org_packed_date 
ON outbound_to_data(organization_id, packed_by, packed_at);

CREATE INDEX IF NOT EXISTS idx_outbound_org_shipped_date 
ON outbound_to_data(organization_id, shipped_by, shipped_at);

CREATE INDEX IF NOT EXISTS idx_outbound_org_final_packed_date 
ON outbound_to_data(organization_id, final_packed_by, final_packed_at);

-- Putback tickets composite index
CREATE INDEX IF NOT EXISTS idx_putback_org_created_date 
ON putback_tickets(organization_id, created_by, created_at);

-- Cycle count composite index
CREATE INDEX IF NOT EXISTS idx_cyclecount_org_assigned_completed 
ON rr_cyclecount_data(organization_id, assigned_to, completed_at);

-- =========================================================================
-- PART 2: AGGREGATED PRODUCTIVITY COUNTS RPC
-- Returns all associate productivity counts in a single query
-- Replaces 8 × N individual count queries with 1 query
-- =========================================================================
...
-- =========================================================================
-- PART 3: AGGREGATED ACTIVITY EVENTS RPC
-- Returns all activity events with timestamps for all associates in a single query
-- Replaces 9 × N individual data queries with 1 query
-- =========================================================================
...
-- =========================================================================
-- PART 4: WEEKLY PRODUCTIVITY SUMMARY RPC
-- Returns aggregated daily productivity for the past 7 days in a single query
-- Replaces 7 × full getTeamProductivity calls with 1 query
-- =========================================================================
...
BEGIN
  ...
  daily_inbound AS (
    SELECT 
      ...
      COUNT(DISTINCT ris.scanned_by) AS users_with_activity,
      COUNT(*) AS cnt
    FROM rr_inbound_scans ris
    WHERE ris.organization_id = p_organization_id
  ...
  -- Count total and active associates per day
  daily_associates AS (
    SELECT 
      ds.day_date,
      (SELECT COUNT(*) FROM active_users) AS total_associates,
      COALESCE(di.users_with_activity, 0) AS active_associates
    FROM date_series ds
...
-- Grant execute permission to authenticated users
GRANT EXECUTE ON FUNCTION get_weekly_productivity_summary(UUID, DATE) TO authenticated;

-- =========================================================================
-- PART 5: SHIFT ASSIGNMENTS WITH FULL DETAILS RPC
-- Returns all shift assignments with joined user, position, area, and schedule data
-- Replaces the complex nested select with a single optimized query
-- =========================================================================
...
-- Grant execute permission to authenticated users
GRANT EXECUTE ON FUNCTION get_shift_assignments_with_details(UUID) TO authenticated;
...
'Returns aggregated productivity task counts for all active associates in an organization for a date range. Eliminates N+1 queries by fetching all counts in a single query.';

COMMENT ON FUNCTION get_team_activity_events IS 
'Returns all activity events with timestamps for all active associates. Used for building Gantt timeline visualizations. Eliminates N+1 queries.';

COMMENT ON FUNCTION get_weekly_productivity_summary IS 
'Returns aggregated daily productivity summary for the past 7 days. Replaces 7 separate getTeamProductivity calls with a single optimized query.';

COMMENT ON FUNCTION get_shift_assignments_with_details IS 
'Returns all shift assignments with fully joined user, position, area, schedule, and supervisor details in a single query.';
