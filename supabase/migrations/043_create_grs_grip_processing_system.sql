-- Create GRS GRIP Processing System
-- Migration: 043_create_grs_grip_processing_system.sql
-- Description: Creates rr_grsgrip_processing table and get_grs_grip_processing_statistics RPC function
-- Date: 2025-09-24

-- Create the rr_grsgrip_processing table
CREATE TABLE IF NOT EXISTS public.rr_grsgrip_processing (
  ...
  -- Core GRS GRIP processing fields
...
CREATE INDEX IF NOT EXISTS idx_rr_grsgrip_processing_organization_id ON public.rr_grsgrip_processing(organization_id);
CREATE INDEX IF NOT EXISTS idx_rr_grsgrip_processing_created_by ON public.rr_grsgrip_processing(created_by);
CREATE INDEX IF NOT EXISTS idx_rr_grsgrip_processing_material_number ON public.rr_grsgrip_processing(material_number);
CREATE INDEX IF NOT EXISTS idx_rr_grsgrip_processing_batch_number ON public.rr_grsgrip_processing(batch_number);
CREATE INDEX IF NOT EXISTS idx_rr_grsgrip_processing_warehouse_number ON public.rr_grsgrip_processing(warehouse_number);
CREATE INDEX IF NOT EXISTS idx_rr_grsgrip_processing_processing_status ON public.rr_grsgrip_processing(processing_status);
CREATE INDEX IF NOT EXISTS idx_rr_grsgrip_processing_created_at ON public.rr_grsgrip_processing(created_at);

-- Enable RLS
ALTER TABLE public.rr_grsgrip_processing ENABLE ROW LEVEL SECURITY;

-- Create RLS policies
CREATE POLICY "Users can view GRS GRIP processing operations for their organization"
  ON public.rr_grsgrip_processing FOR SELECT
  USING (organization_id = auth.jwt() ->> 'organization_id'::text);

CREATE POLICY "Users can insert GRS GRIP processing operations for their organization"
  ON public.rr_grsgrip_processing FOR INSERT
  WITH CHECK (organization_id = auth.jwt() ->> 'organization_id'::text);

CREATE POLICY "Users can update GRS GRIP processing operations for their organization"
  ON public.rr_grsgrip_processing FOR UPDATE
  USING (organization_id = auth.jwt() ->> 'organization_id'::text);

CREATE POLICY "Users can delete GRS GRIP processing operations for their organization"
  ON public.rr_grsgrip_processing FOR DELETE
  USING (organization_id = auth.jwt() ->> 'organization_id'::text);

-- Create get_grs_grip_processing_statistics RPC function
