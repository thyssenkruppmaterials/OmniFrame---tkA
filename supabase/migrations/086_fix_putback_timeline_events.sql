-- =========================================================================
-- Fix Putback Timeline Events Migration
-- Created: January 4, 2026
-- Purpose: Fix putback events not appearing on Gantt timeline when 
--          processed on a different date than created
-- Issue: get_team_activity_events RPC filtered putbacks by created_at
--        but used COALESCE(processed_at, created_at) for event timestamp
-- Solution: Filter by the actual event timestamp date, not just created_at
-- =========================================================================

-- Drop the existing function
DROP FUNCTION IF EXISTS get_team_activity_events(UUID, TIMESTAMPTZ, TIMESTAMPTZ);

-- Recreate with fixed putback query
...
BEGIN
  ...
  -- Putbacks (processed) - FIXED: Split into two queries for proper date filtering
  -- This ensures putbacks appear on the timeline for the date they were processed
  SELECT 
    pt.created_by AS user_id,
    'putback'::TEXT AS event_type,
    pt.processed_at AS event_timestamp,
    'Putback'::TEXT AS area
  FROM putback_tickets pt
  WHERE pt.organization_id = p_organization_id
    AND pt.processed_at IS NOT NULL
    AND pt.processed_at >= p_start_date
    AND pt.processed_at <= p_end_date
    AND pt.created_by IN (SELECT au.user_id FROM active_users au)
  
  UNION ALL
  
  -- Putbacks (not yet processed) - Use created_at if processed_at is null
  SELECT 
    pt.created_by AS user_id,
    'putback'::TEXT AS event_type,
...
'Returns all activity events with timestamps for all active associates. Used for building Gantt timeline visualizations. Eliminates N+1 queries. FIXED: Putback events now filtered by processing date to match timeline display.';
