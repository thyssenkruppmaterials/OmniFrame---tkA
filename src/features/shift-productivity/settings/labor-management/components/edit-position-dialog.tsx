...
export function EditPositionDialog({
  ...
  const { shiftPositions, updateShiftPosition } = useLaborManagement()
  ...
  useEffect(() => {
    if (position && open) {
      const skillsString = Array.isArray(position.required_skills)
        ? position.required_skills.join(', ')
        : ''
      const certificationsString = Array.isArray(
        position.required_certifications
      )
        ? position.required_certifications.join(', ')
        : ''
      ...
      form.reset({
        ...
        reports_to_position_id: position.reports_to_position_id || '__none__',
        headcount_budget: position.headcount_budget,
        is_supervisory: position.is_supervisory,
        is_active: position.is_active,
        minimum_experience_years: position.minimum_experience_years || 0,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='max-h-[85vh] w-[95vw] max-w-[1400px] min-w-[1200px] overflow-y-auto'>
        <DialogHeader>
          ...
          <DialogDescription>
            Update position details, reporting relationships, and requirements.
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-8'>
            ...
            <div className='space-y-5'>
              ...
              {/* Row 1: Code, Title (col-span-2), Department */}
              ...
              {/* Row 2: Type, Level, Budget, Reports To */}
              <div className='grid grid-cols-4 gap-6'>
                <FormField
                  control={form.control}
                  name='position_type'
                    ...
                    <FormItem>
                      ...
                      >
                        ...
                        <SelectContent position='popper' sideOffset={5}>
                          ...
                          {shiftPositions
                            .filter(
                              (p) => p.is_supervisory && p.id !== position?.id
                            )
                            .sort((a, b) => b.position_level - a.position_level)
                            .map((pos) => (
                              <SelectItem key={pos.id} value={pos.id}>
                                L{pos.position_level} - {pos.position_title}
                              </SelectItem>
                            ))}
                        </SelectContent>
                      </Select>
            ...
            <div className='space-y-5'>
              ...
              {/* Bottom row: Min Experience, Supervisory, Active */}
              <div className='grid grid-cols-4 gap-6'>
                <FormField
                  control={form.control}
