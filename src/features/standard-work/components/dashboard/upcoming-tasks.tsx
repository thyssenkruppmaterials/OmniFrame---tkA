 ...
 *
 * Collapsible per-day grouping of tasks scheduled for the next N days.
 * The subtitle now reports the configured window (e.g. 7 days) and the
 * count of days that actually have work, so the copy doesn't mislead users
 * when only one day in the window has tasks.
 */
...
interface UpcomingTasksSectionProps {
  ...
  isError?: boolean
  onRetry?: () => void
  /** The configured window in days that the parent queries for. */
  windowDays?: number
}
...
export function UpcomingTasksSection({
  ...
  const scheduledDays = upcomingTasks.length
          ...
          <div className='flex items-center gap-3'>
            ...
            <div className='min-w-0 flex-1'>
              ...
              <CardDescription className='text-xs'>
                Next {windowDays} day{windowDays === 1 ? '' : 's'}
                {totalUpcoming > 0 ? (
                  <>
                    {' · '}
                    {totalUpcoming} task{totalUpcoming === 1 ? '' : 's'}
                    {' across '}
          ...
          {isError ? (
            <Alert variant='destructive'>
              ...
              <AlertDescription className='flex items-center justify-between gap-2'>
                <span className='text-sm'>Couldn't load the schedule.</span>
                {onRetry && (
                  <Button
                    size='sm'
                    variant='outline'
            ...
            <div className='flex flex-col items-center justify-center py-10 text-center'>
              ...
              <p className='text-muted-foreground/70 mt-0.5 text-xs'>
                Your next scheduled work will appear here
              </p>
            </div>
          ) : (
              ...
              {upcomingTasks.map(({ date, tasks }) => {
                ...
                return (
                  <Collapsible
                    ...
                    open={isOpen}
                    onOpenChange={(open) =>
                      setOpenDates((prev) => ({ ...prev, [date]: open }))
                    }
                  >
                    <CollapsibleTrigger
                      className='hover:bg-accent/50 group flex w-full items-center justify-between rounded-lg border p-3 transition-colors'
                    ...
                    <CollapsibleContent>
                      <div className='border-muted ml-4 space-y-1.5 border-l-2 py-2 pl-4'>
                        {tasks.map((task) => (
                          <div
                            key={`${task.template_id}-${task.working_area_id ?? 'all'}`}
