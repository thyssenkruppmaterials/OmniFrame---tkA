...
import type { ImportProgress } from '@/lib/supabase/lx03-data.service'
...
interface LX03ImportProgressDialogProps {
  isOpen: boolean
  progress: ImportProgress | null
  onClose?: () => void
}

export const LX03ImportProgressDialog = React.memo(
  ({ isOpen, progress, onClose }: LX03ImportProgressDialogProps) => {
    ...
    const progressValue = React.useMemo(() => {
      ...
      switch (progress.phase) {
        ...
        case 'inserting':
          // For backend API imports, use insertedRows instead of processedChunks
          // This supports the simulated progress updates
          if (progress.insertedRows > 0 && progress.totalRows > 0) {
            return 55 + (progress.insertedRows / progress.totalRows) * 40
          }
          // Fallback to chunk-based calculation for client-side imports
      ...
      () => (
        <DialogHeader>
          <DialogTitle className='flex items-center gap-2'>
            {phaseIcon}
            LX03 Import Progress
          </DialogTitle>
          <DialogDescription>
            Processing {progress.totalRows.toLocaleString()} LX03 data records
          </DialogDescription>
        </DialogHeader>
      ),
            ...
            {isCompleted && (
              <div className='rounded-lg bg-green-50 p-3 dark:bg-green-900/20'>
                <p className='text-sm font-medium text-green-700 dark:text-green-300'>
                  🎉 LX03 Import Completed Successfully!
                </p>
