...
//! `LISTEN cycle_count_data_changed` consumer.
//!
//! Spawned once at boot. On each Postgres NOTIFY (emitted by the
//! trigger added in migration 273), parses the JSON payload, builds a
//! `WsEvent::CycleCountOperationChanged`, and broadcasts it via the
//! existing `broadcast::Sender<WsEvent>` so all connected WS clients
//! with a matching org subscription receive it.
//!
//! Replaces the org-filtered
//! `supabase.channel('cycle-count-changes-{orgId}')` callsite in
//! `src/hooks/use-cycle-count-operations.ts`. See
//! `memorybank/OmniFrame/Implementations/Migrate-Tier1-Deferred-Channels-To-Rust-WS.md`.
//!
//! High-frequency table — the trigger fires per cycle-count UPDATE
//! (every claim, every status flip, every variance recalc). Logging is
//! `tracing::debug!` per-event because at fleet scale this can be
...
pub const CHANNEL: &str = "cycle_count_data_changed";
...
pub async fn handle(frame: &NotifyFrame, ws_tx: &broadcast::Sender<WsEvent>) {
    match serde_json::from_str::<Notification>(&frame.payload) {
        Ok(n) => {
            let event = WsEvent::CycleCountOperationChanged {
                row_id: n.row_id,
                organization_id: n.organization_id,
                op: n.op,
            };
            if let Err(e) = crate::websocket::broadcast_event(ws_tx, event) {
                debug!(?e, "cycle_count_listener: no WS subscribers (ignored)");
            }
        }
        Err(e) => {
            warn!(
                ...
                "cycle_count_listener: bad payload (skipped)"
            );
        }
    }
}
